...
function psp2_territorial_admin_page(): void {
    ...
    if ( isset( $_POST['psp2_terr_save'] ) && check_admin_referer( 'psp2_territorial_settings' ) ) {
        ...
        $modo_new = sanitize_key( wp_unslash( $_POST['modo'] ?? 'pspv2_rest' ) );
        if ( ! in_array( $modo_new, [ 'json_url', 'pspv2_rest' ], true ) ) {
            $modo_new = 'pspv2_rest';
        }
    ...
    <div class="wrap">
      ...
      <form method="post">
        ...
        <table class="form-table">
          <tr id="psp2-json-row" <?php echo get_option( 'psp2_territorial_modo', 'pspv2_rest' ) !== 'json_url' ? 'style="display:none"' : ''; ?>>
          ...
          <tr>
            ...
            <td>
              <select name="modo" id="psp2_terr_modo" onchange="document.getElementById('psp2-json-row').style.display=this.value==='json_url'?'':'none'">
                <option value="json_url"   <?php selected( get_option( 'psp2_territorial_modo', 'pspv2_rest' ), 'json_url' ); ?>>JSON externo (URL)</option>
                <option value="pspv2_rest" <?php selected( get_option( 'psp2_territorial_modo', 'pspv2_rest' ), 'pspv2_rest' ); ?>>PSP Territorial V2 (REST local)</option>
              </select>
              <p class="description">
