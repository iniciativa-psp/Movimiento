...
add_action( 'admin_notices', 'psp_erp_check_core' );
function psp_erp_check_core() {
    if ( ! class_exists( 'PSP_Supabase' ) )
        echo '<div class="notice notice-error"><p><strong>psp-erp:</strong> Requiere <strong>PSP Core</strong> activo.</p></div>';
}

add_action( 'plugins_loaded', 'psp_erp_late_init' );
function psp_erp_late_init() {
